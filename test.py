...
def show_welcome_page():
    ...
    with col2:
        ...
        st.markdown("<h1 style='text-align: center;'>مرحباً بك في DZ-UniGuide AI</h1>", unsafe_allow_html=True)
        
        # المقولة والنصيحة التي طلبتها
        st.markdown("""
        <div style='text-align: center; background-color: #f0f2f6; padding: 25px; border-radius: 15px; border-right: 5px solid #4CAF50;'>
            <h3 style='color: #2e7d32;'>✨ "اختر ما تحب، فالشغف هو سر النجاح"</h3>
            <p style='font-size: 18px;'>رحلتك الجامعية تبدأ من هنا.. نحن نساعدك على اختيار المسار الذي يناسب قدراتك وطموحك.</p>
        </div>
        """, unsafe_allow_html=True)
        
        st.markdown("<br>", unsafe_allow_html=True)
        
        # زر الانتقال للواجهة التالية
...
with st.sidebar:
    if st.button("🏠 العودة للرئيسية"):
        # تنظيف بيانات الجلسة للبدء من جديد
        for key in ['final_db', 'filtered_res', 'display_df']:
...
# إذا كنا في صفحة الترحيب، اعرضها ثم توقف عن قراءة باقي الكود
if st.session_state.page == "welcome":
    show_welcome_page()
    st.stop() # هذا يمنع ظهور باقي التطبيق حتى يضغط المستخدم على "التالي"
